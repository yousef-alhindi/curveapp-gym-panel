...
export class OfferMgmtComponent implements OnDestroy {
  ...
  joinNewOffer(item: any) {
    if (item.joinFee != 0) {
      ...
      this.api.post1("offer/saveOrder", data).subscribe({
        next: (res: any) => {
          this.successDialog("500ms", "500ms");
          this.getnewOfferList(this.enrooledStatus ? this.enrooledStatus : 2)
        },
        error: (err: any) => {
          console.log(err);
          this.toaster.error(err["error"]["message"]);
  ...
  toggleEvent(d: any, id: any) {
    ...
    this.api.post1("offer/enrolledCategory/changeStatus", data).subscribe({
      next: (res: any) => {
        if (d?.checked) {
          this.toaster.success("Enrolled category Unblocked successfully");
        } else {
          this.toaster.error("Enrolled category Blocked successfully");
        }
        this.ngOnInit();
      },
      error: (err: any) => {
