...
export class AddRestaurantLocationComponent {
  ...
  ngOnInit() {
    ...
    if (this.restaurantData) {
      ...
      this.url = this.restaurantData?.addressDetails?.gymLogo
      this.profileImage = this.restaurantData?.addressDetails?.gymLogo
    }
  }
  ...
  RestaurantAddressForm: FormGroup = new FormGroup({
    ...
    gymLogo: new FormControl("", [Validators.required]),
  });
  ...
  addResturantAddress() {
    ...
    data.gymLogo = this.restaurantData?.addressDetails?.gymLogo ? this.restaurantData?.addressDetails?.gymLogo : this.profileImage;
    this.api.put("addGymLocation", data).subscribe({
      next: (res: any) => {
        this.session.setSessionStorage(
          "curve-restaurants",
          JSON.stringify(res.data)
        );
        this.toastr.success("Gym location added successfully ");
