...
export class UploadDocumentsComponent implements OnInit {
  ...
  constructor(
    ...
    this.isReUpload = this.router.url.includes('Re-Upload');
  }
  ...
  adddocumentResturants() {
    ...
    // debugger
    if(this.addResturantDocumentsForm?.value?.license == '' || this.addResturantDocumentsForm?.value?.idProof == ''){
      this.toastr.error('License and Id Proof both are required')
      return
    }
    ...
    this.api.put("addGymDocuments", data).subscribe({
      next: (res: any) => {
        this.session.setSessionStorage(
          "curve-restaurants",
          JSON.stringify(res.data)
        );
        this.toastr.success("Gym document details added successfully!");
