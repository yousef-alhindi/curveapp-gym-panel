...
export class OtpComponent implements OnInit {
  ...
  verifyOtp() {
    ...
    this.api.post("otp", { otp: this.otp }).subscribe({
      next: (res: any) => {
        console.log();
        ...
        if (
          res.data.gymStatus == 2 &&
          res.data.rejected_reason.rejectedBy == "Document Rejected"
        ) {
          this.toaster.error(res.message);
          this.router.navigateByUrl("/upload-documents/Re-Upload");
          return;
        }
        if (
          res.data.gymStatus == 2 &&
        ...
        }else if (res.data.gymStatus == 0) {
          this.toaster.warning("Your profile is Pending");
